//! `CREATE TABLE` / `DROP TABLE` / multi-row `INSERT` for CSV import into Postgres.
//!
//! Inserts use **bound parameters** — cell data is never spliced into SQL text,
//! so it can never inject. Postgres uses **positional** placeholders (`$1`, `$2`,
//! …, NOT `?`) and caps a statement at 65535 bound parameters, so each `INSERT`
//! is sub-batched to at most `65535 / column_count` rows (see
...
/// Postgres' bound-parameter cap per statement (a 16-bit count → 65535).
const MAX_PARAMS: usize = 65535;

/// Build a double-quoted, optionally schema-qualified table name.
...
/// characters that can legitimately appear in a Postgres column type are allowed,
/// so the fragment cannot break out of the type position.
...
/// type fragment is validated. `database` is ignored — a Postgres connection
/// always targets its connected database.
...
pub(super) async fn drop_table(
    ...
    let ddl = format!("DROP TABLE {}", qualify(schema, table));
    run_batch(conn, &ddl).await
}

/// Run a parameterless statement for its side effects.
async fn run_batch(conn: &mut PgConnection, sql: &str) -> Result<(), CoreError> {
...
pub(super) async fn import_rows(
    ...
    let qualified = qualify(&target.schema, &target.table);
    let col_list = target
        .columns
        .iter()
        .map(|c| quote_ident(c))
    ...
    if atomic {
        // A real Postgres transaction: `begin()` returns a guard we explicitly
        // commit on success or roll back (best-effort) on the first error. A
        // `Transaction` derefs to `PgConnection`, so the insert loop runs against
        // `&mut *tx` and the same code path serves both modes.
        let mut tx = conn.begin().await.map_err(map_sqlx_err)?;
        let outcome = insert_loop(
            &mut tx, // &mut Transaction → &mut PgConnection via DerefMut
            source,
            &qualified,
            &col_list,
            col_count,
...
/// Placeholders are positional (`$1`, `$2`, …) and number across the whole
/// statement.
async fn insert_chunk(
    ...
    for (ri, row) in rows.iter().enumerate() {
        ...
        for ci in 0..col_count {
            ...
            // `write!` to a String is infallible. Positional `$N` placeholders.
            let _ = write!(sql, "${p}");
            p += 1;
        }
        sql.push(')');
    ...
    // Bind every cell in row-major order to match the `$N` placeholders.
    ...
    // `&mut PgConnection` is the sqlx `Executor` for a single statement.
